...
weight1.setConfig("000", "111", "0", write=True) # Set voltage range to smallest, i.e. most sensitive. Set to continuous read mode, so need to write new config now.
weightSensor.calibrateSensors(weight1, mass=100, mass2=369)
...
def bird_detect_fn():
    ...
    while True:
        ...
        (w, w_g, bird_detect, capture_time, image_file_name, file_path) = weightSensor.loop(weight1, 2)
        print(bird_detect)
